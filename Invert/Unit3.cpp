	...
	: TForm(Owner)
{
	...
	for (int j = Form1->iNullFirst; j < Form1->iNullSecond ; j++)
	{
		...
		{
		dAmplituda += (StrToFloat(Form1->StringGrid2->Cells[2][k])*cos(2*M_PI*k*(j-Form1->iNullFirst+1)/(Form1->iNullSecond-Form1->iNullFirst)) + StrToFloat(Form1->StringGrid2->Cells[1][k])*sin(2*M_PI*k*(j-Form1->iNullFirst+1)/(Form1->iNullSecond-Form1->iNullFirst)));
		}
